...
function Chatbot() {
  const [messages, setMessages] = useState([
    { sender: "bot", text: "I am EventEaseBot. How can I help you?" }
  ]);
  const [input, setInput] = useState("");

  const isEventEaseQuestion = (text) => {
    const keywords = [
      "eventease", "event ease", "event", "feature", "route", "how", "register", "login", "create event", "add event", "platform", "utilities", "utility", "services"
    ];
  ...
  const sendMessage = async (e) => {
    ...
    let botReply;
    if (
      input.toLowerCase().includes("utilities") ||
      input.toLowerCase().includes("utility")
    ) {
      botReply = (
        <span>
          EventEase provides the following utilities:
          <ul>
            <li>
              <b>Browse Events:</b> <Link to="/events" style={{ color: "#1976d2" }}>/events</Link>
            </li>
            <li>
              <b>View Event Details:</b> <Link to="/events/:eventId" style={{ color: "#1976d2" }}>/events/:eventId</Link>
            </li>
            <li>
              <b>Add Event:</b> <Link to="/addevent" style={{ color: "#1976d2" }}>/addevent</Link> (requires login)
            </li>
            <li>
              <b>Login:</b> <Link to="/login" style={{ color: "#1976d2" }}>/login</Link>
            </li>
            <li>
              <b>Signup:</b> <Link to="/signup" style={{ color: "#1976d2" }}>/signup</Link>
            </li>
            <li>
              <b>Chatbot:</b> <Link to="/chatbot" style={{ color: "#1976d2" }}>/chatbot</Link>
            </li>
          </ul>
        </span>
      );
    } else if (isEventEaseQuestion(input)) {
